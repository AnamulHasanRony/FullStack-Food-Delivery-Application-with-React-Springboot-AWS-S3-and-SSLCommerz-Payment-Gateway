...
const ExploreMenu = ({category,setCategory}) => {
  ...
  const scrollLeft=()=>{
    if(menuRef.current){
      menuRef.current.scrollBy({left:-230,behavior:'smooth'});
    }
  };
  ...
  return (
    <div className="explore-menu position-relative">
        <h1 className="d-flex alifn-items-center justify-content-between">
            Explore our Menu
            ...
            categories.map((item,index)=>{
              return(
                <div key={index} className='text-center explore-menu-list-item' onClick={()=>setCategory(prev=>prev===item.category ?'All':item.category)}>
                  <img src={item.icon} alt="" className={item.category===category? 'rounded-circle active':'rounded-circle'} height={200} width={200}/>
